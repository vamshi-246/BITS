...
module bm_radix4 (
    ...
    assign bm_r4_0  = $signed(bm_r2_even_0)  + $signed(bm_r2_odd_0);  // even[0][0] + odd[0][0]
    // s''=001, s'=000: odd[0][pred_idx(001 in 000)=1] + even[0][0]
    assign bm_r4_1  = $signed(bm_r2_even_1)  + $signed(bm_r2_odd_0);  // even[0][1] + odd[0][0]
    // s''=010, s'=001: odd[1][pred_idx(010 in 001)=0] + even[0][pred_idx(001 in 000)=1]
    assign bm_r4_2  = $signed(bm_r2_even_2)  + $signed(bm_r2_odd_1);  // even[1][0] + odd[0][1]
    // s''=011, s'=001: odd[1][pred_idx(011 in 001)=1] + even[0][1]
    assign bm_r4_3  = $signed(bm_r2_even_3)  + $signed(bm_r2_odd_1);  // even[1][1] + odd[0][1]

    // --- State 001 (dest=1) ---
    // s''=100, s'=010: odd[2][pred_idx(100 in 010)=0] + even[1][pred_idx(010 in 001)=0]
    assign bm_r4_4  = $signed(bm_r2_even_4)  + $signed(bm_r2_odd_2);  // even[2][0] + odd[1][0]
    // s''=101, s'=010: odd[2][pred_idx(101 in 010)=1] + even[1][0]
    assign bm_r4_5  = $signed(bm_r2_even_5)  + $signed(bm_r2_odd_2);  // even[2][1] + odd[1][0]
    // s''=110, s'=011: odd[3][pred_idx(110 in 011)=0] + even[1][pred_idx(011 in 001)=1]
    assign bm_r4_6  = $signed(bm_r2_even_6)  + $signed(bm_r2_odd_3);  // even[3][0] + odd[1][1]
    // s''=111, s'=011: odd[3][pred_idx(111 in 011)=1] + even[1][1]
    assign bm_r4_7  = $signed(bm_r2_even_7)  + $signed(bm_r2_odd_3);  // even[3][1] + odd[1][1]

    // --- State 010 (dest=2) ---
    // s''=000, s'=100: odd[4][pred_idx(000 in 100)=0] + even[2][pred_idx(100 in 010)=0]
    assign bm_r4_8  = $signed(bm_r2_even_8)  + $signed(bm_r2_odd_4);  // even[4][0] + odd[2][0]
    // s''=001, s'=100: odd[4][pred_idx(001 in 100)=1] + even[2][0]
    assign bm_r4_9  = $signed(bm_r2_even_9)  + $signed(bm_r2_odd_4);  // even[4][1] + odd[2][0]
    // s''=010, s'=101: odd[5][pred_idx(010 in 101)=0] + even[2][pred_idx(101 in 010)=1]
    assign bm_r4_10 = $signed(bm_r2_even_10) + $signed(bm_r2_odd_5);  // even[5][0] + odd[2][1]
    // s''=011, s'=101: odd[5][pred_idx(011 in 101)=1] + even[2][1]
    assign bm_r4_11 = $signed(bm_r2_even_11) + $signed(bm_r2_odd_5);  // even[5][1] + odd[2][1]

    // --- State 011 (dest=3) ---
    // s''=100, s'=110: odd[6][pred_idx(100 in 110)=0] + even[3][pred_idx(110 in 011)=0]
    assign bm_r4_12 = $signed(bm_r2_even_12) + $signed(bm_r2_odd_6);  // even[6][0] + odd[3][0]
    // s''=101, s'=110: odd[6][pred_idx(101 in 110)=1] + even[3][0]
    assign bm_r4_13 = $signed(bm_r2_even_13) + $signed(bm_r2_odd_6);  // even[6][1] + odd[3][0]
    // s''=110, s'=111: odd[7][pred_idx(110 in 111)=0] + even[3][pred_idx(111 in 011)=1]
    assign bm_r4_14 = $signed(bm_r2_even_14) + $signed(bm_r2_odd_7);  // even[7][0] + odd[3][1]
    // s''=111, s'=111: odd[7][pred_idx(111 in 111)=1] + even[3][1]
    assign bm_r4_15 = $signed(bm_r2_even_15) + $signed(bm_r2_odd_7);  // even[7][1] + odd[3][1]

    // --- State 100 (dest=4) ---
    // s''=000, s'=000: odd[0][0] + even[4][pred_idx(000 in 100)=0]
    assign bm_r4_16 = $signed(bm_r2_even_0)  + $signed(bm_r2_odd_8);  // even[0][0] + odd[4][0]
    // s''=001, s'=000: odd[0][1] + even[4][0]
    assign bm_r4_17 = $signed(bm_r2_even_1)  + $signed(bm_r2_odd_8);  // even[0][1] + odd[4][0]
    // s''=010, s'=001: odd[1][0] + even[4][pred_idx(001 in 100)=1]
    assign bm_r4_18 = $signed(bm_r2_even_2)  + $signed(bm_r2_odd_9);  // even[1][0] + odd[4][1]
    // s''=011, s'=001: odd[1][1] + even[4][1]
    assign bm_r4_19 = $signed(bm_r2_even_3)  + $signed(bm_r2_odd_9);  // even[1][1] + odd[4][1]

    // --- State 101 (dest=5) ---
    // s''=100, s'=010: odd[2][0] + even[5][pred_idx(010 in 101)=0]
    assign bm_r4_20 = $signed(bm_r2_even_4)  + $signed(bm_r2_odd_10); // even[2][0] + odd[5][0]
    // s''=101, s'=010: odd[2][1] + even[5][0]
    assign bm_r4_21 = $signed(bm_r2_even_5)  + $signed(bm_r2_odd_10); // even[2][1] + odd[5][0]
    // s''=110, s'=011: odd[3][0] + even[5][pred_idx(011 in 101)=1]
    assign bm_r4_22 = $signed(bm_r2_even_6)  + $signed(bm_r2_odd_11); // even[3][0] + odd[5][1]
    // s''=111, s'=011: odd[3][1] + even[5][1]
    assign bm_r4_23 = $signed(bm_r2_even_7)  + $signed(bm_r2_odd_11); // even[3][1] + odd[5][1]

    // --- State 110 (dest=6) ---
    // s''=000, s'=100: odd[4][0] + even[6][pred_idx(100 in 110)=0]
    assign bm_r4_24 = $signed(bm_r2_even_8)  + $signed(bm_r2_odd_12); // even[4][0] + odd[6][0]
    // s''=001, s'=100: odd[4][1] + even[6][0]
    assign bm_r4_25 = $signed(bm_r2_even_9)  + $signed(bm_r2_odd_12); // even[4][1] + odd[6][0]
    // s''=010, s'=101: odd[5][0] + even[6][pred_idx(101 in 110)=1]
    assign bm_r4_26 = $signed(bm_r2_even_10) + $signed(bm_r2_odd_13); // even[5][0] + odd[6][1]
    // s''=011, s'=101: odd[5][1] + even[6][1]
    assign bm_r4_27 = $signed(bm_r2_even_11) + $signed(bm_r2_odd_13); // even[5][1] + odd[6][1]

    // --- State 111 (dest=7) ---
    // s''=100, s'=110: odd[6][0] + even[7][pred_idx(110 in 111)=0]
    assign bm_r4_28 = $signed(bm_r2_even_12) + $signed(bm_r2_odd_14); // even[6][0] + odd[7][0]
    // s''=101, s'=110: odd[6][1] + even[7][0]
    assign bm_r4_29 = $signed(bm_r2_even_13) + $signed(bm_r2_odd_14); // even[6][1] + odd[7][0]
    // s''=110, s'=111: odd[7][0] + even[7][pred_idx(111 in 111)=1]
    assign bm_r4_30 = $signed(bm_r2_even_14) + $signed(bm_r2_odd_15); // even[7][0] + odd[7][1]
    // s''=111, s'=111: odd[7][1] + even[7][1]
    assign bm_r4_31 = $signed(bm_r2_even_15) + $signed(bm_r2_odd_15); // even[7][1] + odd[7][1]
