...
module tb_turbo_decoder;
    ...
    integer hard_ram_mismatches;
    reg [3:0] expected_hard_row;
    integer meta_fd;
    integer meta_scan;
    integer meta_k;
    ...
    initial begin
        ...
        // ---- Verify production hard-decision readback path ----
        hard_ram_mismatches = 0;
        for (i = 0; i < TEST_ROW_DEPTH; i = i + 1) begin
            @(posedge clk);
            hard_rd_addr <= i[TEST_ROW_ADDR_W-1:0];
            @(posedge clk);
            #1;
            expected_hard_row[0] = (rtl_final_intrinsic[2*i] < 0);
            expected_hard_row[1] = (rtl_final_intrinsic[TEST_SEG_LEN + 2*i] < 0);
            expected_hard_row[2] = (rtl_final_intrinsic[2*i + 1] < 0);
            expected_hard_row[3] = (rtl_final_intrinsic[TEST_SEG_LEN + 2*i + 1] < 0);
            if (hard_rd_data !== expected_hard_row) begin
                if (hard_ram_mismatches < 8)
                    $display("ERROR: hard RAM row %0d got %04b expected %04b", i, hard_rd_data, expected_hard_row);
                hard_ram_mismatches = hard_ram_mismatches + 1;
            end
        end
        if (hard_ram_mismatches == 0)
            $display("PASS: Hard-decision RAM matches final intrinsic signs.");
        else
            $display("ERROR: Hard-decision RAM mismatches = %0d rows.", hard_ram_mismatches);
